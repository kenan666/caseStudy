...
class Model1:
    def __init__(self, training=True):
        ...
        #  [[0,0],[1,1],....[6,6]] 与偏移量相加  组成  中心坐标点 shape[7,7,2]
        ...
        # 输出
        self.output = self.build_network(self.image,num_outputs = self.output_size,alpha = Config.alpha_relu,training = training)
    ...
    def build_network(self,images,num_outputs,alpha,keep_prob = Config.dropout,training = True, scope = 'yolo'):
        ...
        with tf.variable_scope(scope):
            with slim.arg_scope([slim.conv2d,slim.fully_connected],activation_fn=leaky_relu(alpha), weights_initializer=slim.xavier_initializer(),
        ...
        def cal_iou(self,boxes1,boxes2,scope = 'iou'):
            with tf.variable_scope(scope):
                ...
                # 重叠 右下角区域
                b = tf.maximum(boxes1[:,:,:,:,:2],boxes2[:,:,:,:,:2])
        ...
        # 损失函数
        def loss_layer(self,predicts,labels,scope = 'loss'):          #  predicts的shape是[batch,7*7*(20+5*2)]     labels的shape是[batch,7,7,(5+20)]
            with tf.variable_scope(scope):
                #  预测种类，boxes 置信度，boxes坐标[x_center,y_center,w,h],坐标都除以image_size归一化,中心点坐标为偏移量，w,h归一化后又开方，目的是使变化更平缓
                ...
                #  boxes 坐标出路 变成 [batch，7,7,2,4]，两个box最终选一个最高的，为了使 预测更加准确
                boxes = tf.reshape(labels[:,:,:,1:5],self.batch_size,self.cell_size,self.cell_size,1,4)
                boxes = tf.tile(boxes,[1,1,1,self.box_per_cell,1]) / self.image_size
                classes = labels[:,:,:,5:]

                # 利用  offset   [[[0,0],[1,1]...[6,6]],[[0,0]...[6,6]]...]与偏移量x相加
                #offset转置 [[0,0,[0,0]...],[[1,1],[1,1]...],[[6,6]...]]与偏移量y相加
                ...
                predict_boxes_tran=tf.stack([(predict_boxes[:,:,:,:,0]+offset)/self.cell_size,(predict_boxes[:,:,:,:,1]+tf.transpose(offset,(0,2,1,3)))/self.cell_size,
                                            tf.square(predict_boxes[:,:,:,:,2]),tf.square(predict_boxes[:,:,:,:,3])],axis=-1)

                #  iou  中的shape 是 [batch ,7,7,2]
                iou_predict_truth = self.cal_iou(predict_boxes_tran,boxes)

                #  选择 预选框 中 iou 最大的
                object_mask = tf.reduce_mean(iou_predict_truth,3,keep_dims=True)

                #  真是图像中 有预选框，并且两个 选出两个 预选框中  最大的
                ...
                #  真实 boxes 的偏移量
                boxes_tran=tf.stack([boxes[:,:,:,:,0]*self.cell_size-offset,boxes[:,:,:,:,1]*self.cell_size-tf.transpose(offset,(0,2,1,3)),
