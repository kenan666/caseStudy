...
def read_and_decode(filename):
    ...
    _, serialized_example = reader.read(filename_queue)   #  返回文件名  和  文件
...
train_img,train_label = read_and_decode([train_path + 'train.tfrecoreds'])

#使用shuffle随即打乱输入
train_img_batch ,train_label_batch = tf.train.shuffle_batch([train_img,train_label],batch_size=Config.batch_size,
                                                                capacity=10000,min_after_dequeue= 500)
...
val_img_batch,val_label_batch = tf.train.shuffle_batch([val_img, val_label],batch_size=Config.batch_size, capacity=1000,
